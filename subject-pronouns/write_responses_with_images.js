...
function startGame() {
    ...
    function render() {
        ...
        if (currentPage in checkedAnswers) {
            $('.story input').attr('disabled', 'disabled');
            ...
            if (checkedAnswers[currentPage] === 1) {
                $('button:contains("story")').removeAttr('disabled');
            }
        }
    }
    ...
    async function checkResponses() {
        ...
        if ($('.correct').length/$('.story input').length >= 0.8) {
            ...
            // IF ALL PAGES CHECKED
            if (Object.keys(checkedAnswers).length === 7) {
                ...
                if (!sublevelWon) {
                    if (containsValueZero(checkedAnswers)) {
                        ...
                        $('#message-modal .modal-body').html('<p>Nice job! &#128522</p>')
                        // $('#message-modal .modal-body').append(`<button id="restartButton" class="btn-blue btn">Play again</button>`)
                    }
